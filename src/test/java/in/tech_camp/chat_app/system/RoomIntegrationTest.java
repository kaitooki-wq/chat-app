...
import in.tech_camp.chat_app.form.RoomForm; // ★追加：RoomFormのインポート
...
public class RoomIntegrationTest {
  ...
  private RoomEntity roomEntity1; // ★この名前に合わせます
  ...
  public void setup() {
    ...
    // ★修正：UserForm 型を追加
    UserForm userForm2 = UserFormFactory.createUser();
    // ★修正：UserEntity 型を追加
    ...
    // ★修正：RoomForm 型を追加
    RoomForm roomForm = RoomFormFactory.createRoom();
    // ★修正：roomEntity から roomEntity1 に名前を変更
    roomEntity1 = new RoomEntity();
    roomEntity1.setName(roomForm.getName());
    roomRepository.insert(roomEntity1);

    RoomUserEntity roomUserEntity1 = new RoomUserEntity();
    roomUserEntity1.setRoom(roomEntity1); // ★修正：roomEntity1 に変更
    ...
    roomUserEntity2.setRoom(roomEntity1); // ★修正：roomEntity1 に変更
  ...
  public void チャットルームを削除すると関連するメッセージがすべて削除されている() throws Exception {
    ...
    // 作成したチャットルームへ遷移する
    mockMvc.perform(get("/rooms/{roomId}/messages", roomEntity1.getId()).session(session))
    ...
    mockMvc.perform(post("/rooms/{roomId}/delete", roomEntity1.getId()).session(session).with(csrf()))
